...
import { ChevronLeft, ChevronRight, Grid, List, Search, Filter, X } from 'lucide-react';
...
const GalleryManager = () => {
  ...
  const filteredGalleries = galleries.filter(gallery => {
    ...
    const matchesCategory = filterCategory === 'all' || 
      (gallery.category && gallery.category.toLowerCase() === filterCategory.toLowerCase());
    
    return matchesSearch && matchesCategory;
  });

  // Categories derived from gallery data
  const categories = ['all', ...new Set(galleries
    ...
    <Container>
      <div className="space-y-6 py-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <h1 className="text-3xl font-bold text-green-800">Landscaping Gallery</h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex border border-gray-300 rounded-lg overflow-hidden">
              ...
              <button
                ...
                className={`px-3 py-2 ${viewMode === 'list' ? 'bg-green-600 text-white' : 'bg-white text-gray-700'}`}
  ...
  <div className="w-full h-full flex items-center justify-center bg-gray-100">
    <span className="text-gray-400">No image</span>
                  ...
                  <div className="p-4 flex-grow">
                    ...
                    {gallery.description && (
                      <p className="text-sm text-gray-600 mt-2 line-clamp-2">{gallery.description}</p>
                    )}
                  </div>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-90 z-50 flex items-center justify-center">
          <div className="relative w-full h-full flex flex-col">
            ...
            <div className="flex-grow flex items-center justify-center p-4">
              {selectedGallery.images && selectedGallery.images[currentImageIndex] && (
                <img
            ...
            <div className="bg-white p-4 md:p-6">
              <div className="container mx-auto">
                <h2 className="text-xl md:text-2xl font-bold text-gray-900">{selectedGallery.title}</h2>
                ...
                {selectedGallery.description && (
                  <p className="text-gray-700 mt-3">{selectedGallery.description}</p>
                )}
                      ...
                      {selectedGallery.images.map((image, index) => (
                        <button
                          ...
                          className={`flex-shrink-0 w-16 h-16 rounded-md overflow-hidden border-2 ${
                            currentImageIndex === index ? 'border-green-600' : 'border-transparent'
                          }`}
                        >
                          <img 
                            src={image.url} 
